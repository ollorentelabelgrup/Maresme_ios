...
struct MyPropertiesView: View {
    @State private var vm = MyPropertiesViewModel()
    ...
    private var propertyList: some View {
        List {
            ForEach(vm.properties) { property in
                NavigationLink {
                    AgencyPropertyDetailView(slug: property.slug)
                } label: {
                    AgencyPropertyRowView(property: property)
                }
                .listRowBackground(Color.maresmeSurface)
                .onAppear {
                    if vm.isLastItem(property) {
    ...
    private var emptyState: some View {
        VStack {
            ...
            EmptyStateView(
                ...
                message: "No hay propiedades en este estado todavía."
            )
            Spacer()
        }
    }
